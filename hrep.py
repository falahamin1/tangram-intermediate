"""
Intermediate Tangram — H-Representation PPO Training
=====================================================
State shape: [batch, 4, 15]  (4 pieces × 5 constraints × 3 params)
"""
...
def train_h_rep(episodes: int = 1000,
    ...
    buffer = PPOBuffer(size=HP["steps_per_rollout"], state_shape=(4, 15))
    ...
    for ep in range(start_ep, episodes):
        ...
        for t in range(HP["steps_per_rollout"]):
            ...
            if done:
                ...
                with torch.no_grad():
                    _, last_val = model(raw_next.view(4, 15).unsqueeze(0))
    ...
    best_model = DeepSetActorCritic(input_dim=15, num_pieces=4, num_actions=16)
